...
- **Decision taken:** we hand-rolled `payments/x402.js` to the standard x402 V2 wire
...
- **Security**: `.gitignore` excludes `.env`, `.env.*` (keep `.env.example`), `logs/`,
  `node_modules/`, and key material. No secret has been read or copied. The ASP uses its
  own fresh root `.env` (never the bot's Telegram/autotrade/vault secrets).
