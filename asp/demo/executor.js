// asp/demo/executor.js — the "executor agent" that proves the full A2A loop.
//
// It behaves like another agent in the OKX.AI marketplace: it calls Theia's
// theia_signal skill, pays for it with x402 through the OKX Agentic Wallet, reads
// the signal, and then acts on it (places the trade via the Agentic Wallet). This
// is the loop the demo video shows end to end.
//
// Modes:
//   default (dry-run) - drives the loop, decodes the real 402 challenge, and PRINTS
//                       the exact `onchainos` commands it would run. If the target
//                       endpoint serves free (X402_ENFORCE=0) it also fetches the
//                       real signal so the whole flow is visible without a wallet.
//   --live            - actually pays via `onchainos payment pay` and, with
//                       --execute, places the trade via the Agentic Wallet.
//
// Usage:
//   node asp/demo/executor.js --token BTC
//   node asp/demo/executor.js --token SOL --url https://theia-asp.onrender.com --live --execute
...
async function main() {
  ...
  // Step 2 — got a 402 payment challenge.
  ...
  // Step 3 — pay via the OKX Agentic Wallet (x402).
...
function showSignalAndAct(signal) {
  ...
  // Step 5 — act on it via the Agentic Wallet.
