...
Ground truth for all of this is in [asp/NOTES-okx.md](../NOTES-okx.md).
...
1. Install the CLI + skills (already done in this repo, re-runnable):
   ...
   ```
2. Deploy the ASP endpoint FIRST (see [asp/README.md](../README.md) -> Deploy). The
...
```
Put that address in `asp/.env` as `X402_PAY_TO`, and set `ASP_PUBLIC_URL` to your
...
Fill these in `asp/.env` before enforcing x402:
```
onchainos wallet chains            # find the X Layer (196) USDT / USDG token addresses
```
Set `X402_ASSET_USDT_ADDRESS` (and USDG if you want it). For the facilitator URL,
...
```
ASP_PUBLIC_URL=https://theia-asp.onrender.com node asp/identity/build-manifest.js
```
That prints the 6 A2MCP services + the A2A Deep Desk service (also saved to
`asp/identity/service-manifest.json`). Then, in your agent session:
...
- Avatar: send `asp/identity/avatar.png` (add a 1:1 square image here; the ASP avatar is required)
...
`agent create`. Save the returned `#<agentId>` into `asp/.env` as `OKX_ASP_AGENT_ID`.
...
- A2A: stay online; browse tasks or wait for offers. When a task is `job_accepted`
  ...
  [asp/a2a/deep-desk.js](../a2a/deep-desk.js) (`cli.apply`, `cli.deliver`,
